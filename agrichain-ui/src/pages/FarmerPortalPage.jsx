...
import { Leaf, Mail, Lock, User, Home, KeyRound } from 'lucide-react'; // Added User, Home, KeyRound icons

// NOTE: InputField component is moved OUTSIDE the main component 
// to prevent it from being redefined on every render, which often causes focus loss issues.
/**
  ...
  <div className="mb-4">
    ...
    <div className="relative">
      ...
      <input
        ...
        onChange={onChange}
        required
        // Added dark mode classes for input field styling
...
const FarmerPortalPage = () => {
  ...
  // Common fields for both forms (Login uses email, password)
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');

  // Fields specific to Sign Up form
  ...
  const handleSubmit = (e) => {
    ...
    // NOTE: Removed 'alert()' as per constraints; replaced with console log message.
    if (isLogin) {
      // Handle Login logic
  ...
  const CardHeader = () => (
    <div className="flex p-1 bg-gray-100 dark:bg-gray-800 rounded-lg mb-8">
      <button
        type="button" // Important for nested buttons not to submit the form
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      {/* Navbar is rendered by App.jsx, not here */}

      <main className="container flex flex-col items-center justify-start pt-16 pb-20">
        {/* Page Header */}
        ...
        <div className="w-full max-w-md p-8 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700">
          ...
          <form onSubmit={handleSubmit}>
            {!isLogin && ( // Render these fields only if not on Login page
              <>
                <InputField 
                  Icon={User} // User icon for Full Name
                ...
                />
                <InputField 
                  Icon={Home} // Home icon for Aadhar ID 
            ...
            {!isLogin && ( // Render confirm password only for Sign Up
              <InputField 
                Icon={KeyRound} // Key icon for Confirm Password
